...
Inputs (from step1_dataset/ and step2_comparative/):
  ...
  y_labels.csv                   - 1858 samples with MDR/XDR/lineage labels
...
#restricting dr_variants to the aligned 1858 samples only 
...
KEY_MUTS = {
    ...
    # NOTE: matched on `change` string, like every other entry here --
    # NOT on `pos`, which is the genomic coordinate (rrs sits at ~1.47Mb
    # in H37Rv). The "1401" in n.1401A>G is HGVS transcript-relative
    # numbering (the "n." prefix means numbered from the rRNA transcript's
    # own start), not a genomic position -- dr["pos"]==1401 could never
    # match anything and silently returned 0 samples every run.
    "rrs_1401"   : (dr["gene"]=="rrs")  & (dr["change"].str.contains("1401", na=False)),
